...
async function createAuth0User(email, password, name, token) {
  try {
    const response = await fetch(
      'https://dev-oseu3r74.us.auth0.com/api/v2/users',
      {
        method: 'POST',
        headers: {
...
function checkAuthorization() {
  ...
  if (!hasValidParams) {
    console.log(
      'Personal authorization failed - missing required parameters:',
      pageParams
    );
...
function initializePersonalSignin() {
  ...
  console.log(
    'Personal signin initialized successfully with params:',
    pageParams
  );
}
...
signin.addEventListener('submit', e => {
  ...
  (async () => {
    try {
      ...
      const response = await fetch(
        'https://jmrcycling.com:3001/signinPersonal',
        requestOptions
      );
      ...
      const loginResponse = await fetch(
        'https://jmrcycling.com:3001/loginShop',
        requestOptions2
      );
      ...
      sessionStorage.setItem(
        'shop_name',
        loginResult.plan_type[0].shop_name || name
      );
