    ...
    fetch('https://jmrcycling.com:3001/loginShop', requestOptions)
      ...
      .then(result => {
        ...
        window.location.replace(
          './dashboard.html?plan_type=' +
            result.plan_type[0].plan_type +
            '&shop_name=' +
            result.plan_type[0].shop_name +
            '&shop_code=' +
            result.plan_type[0].shop_code
        );
      })
      .catch(error => {
        console.log('error', error);
        window.alert(
          'Your log in was unsuccessful, this could be due to a expired subscription. Please reach out to us if this is not right.'
        );
